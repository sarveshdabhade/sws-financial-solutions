...
function validateInputs() {
  ...
  if (!Number.isFinite(loanAmount) || loanAmount < 0 || loanAmount > 100000000) {
    alert("Please enter a valid loan amount (0-10 crore)");
    return false;
  }
  ...
  if (!Number.isFinite(loanTenure) || loanTenure < 1 || loanTenure > 30) {
    alert("Please enter a valid loan tenure (1-30 years)");
    return false;
  }
  if (!Number.isFinite(currentEMI) || currentEMI < 0) {
    alert("Please enter a valid current EMI");
    return false;
  }
  if (!Number.isFinite(extraSavings) || extraSavings < 0) {
    alert("Please enter valid extra savings");
    return false;
  }
  ...
  if (!Number.isFinite(expectedCAGR) || expectedCAGR < 0 || expectedCAGR > 30) {
    alert("Please enter a valid expected CAGR (0-30%)");
    return false;
  }
  if (!Number.isFinite(investmentDuration) || investmentDuration < 1 || investmentDuration > 30) {
    alert("Please enter a valid investment duration (1-30 years)");
    return false;
  }
  if (!Number.isFinite(inflationRate) || inflationRate < 0 || inflationRate > 30) {
    alert("Please enter a valid inflation rate (0-30%)");
    return false;
  }
  return true;
}
...
function analyzeStrategies() {
  ...
  animateCounters();
    ...
    const errorContainer = document.getElementById('aiRecommendation');
    if (errorContainer) {
      errorContainer.textContent = 'An error occurred during analysis. Please check your inputs and try again.';
    }
  }
}
